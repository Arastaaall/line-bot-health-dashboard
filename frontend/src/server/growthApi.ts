...
function findById(sheetName: string, idColumn: string, idValue: any): Row | null {
  return getRows(sheetName, (row) => String(row[idColumn] ?? '') === String(idValue ?? ''))[0] || null;
}
function toNumber_(value: any, fallback: any = null) {
  if (value === '' || value === null || value === undefined) return fallback;
  const number = Number(value);
  return Number.isFinite(number) ? number : fallback;
}
function toBool_(value: any) {
...
function perfAddProcessing_(_name: string, _ms: number) {}
const __perf = null;
const Utilities = {
  DigestAlgorithm: { MD5: 'MD5' },
  computeDigest: (_algorithm: string, value: string) => Array.from(createHash('md5').update(String(value)).digest()),
};
function cached_(_key: string, _ttl: number, fn: () => any) { return fn(); }
function getUserRecord_(userId: string) {
  const row = getRows('users', (r) => String(r.user_id ?? '') === String(userId))[0];
  return {
    userId,
...
function apiGetTrainingMenus(userId: string, _params: any) {
  const user = getUserRecord_(userId);
  const menus = getRows('Training_Menus', (r) => String(r.user_id ?? '') === String(userId) && toBool_(r.is_active))
    .sort((a, b) => (Number(a.display_order) || 0) - (Number(b.display_order) || 0));
  return { ok: true, data: { menus, limit: user.isPremium ? null : 5 } };
}
function getGoalPlansCached_(userId: string) {
  const plans = getRows('Goal_Plans', (r) => String(r.user_id ?? '') === String(userId));
  let activePlan: Row | null = null;
  const history: Row[] = [];
  plans.forEach((plan) => {
...
function buildTrainingAnalysis_(userId, range) {
  ...
  // T13 月間日数（直近6暦月）
  (function () {
    const now = new Date(`${todayKey_().slice(0, 7)}-01T00:00:00+09:00`);
    const months = [];
    for (let i = 5; i >= 0; i--) {
...
function buildBodyAnalysis_(userId, range, user) {
  ...
  (function () {
    ...
    const now = new Date(`${curM}-01T00:00:00+09:00`);
...
export function growthAction(input: Record<string, Row[]>, userId: string, action: string, params: Record<string, any>) {
  ...
  switch (action) {
    ...
    default: return { ok: false, error: { code: 'MIGRATION_PENDING', message: 'このAPI actionはVercel移行の準備中です' } };
  }
}
